...
import type { LLMChatResponse, LLMProvider, LLMToolCall, LLMMessage, LLMAssistantMessage } from "../types/llm.ts";
import type { RunState } from "../types/run-state.ts";
import type { RunExit } from "./run-exit.ts";
import type { WaitResolution } from "./wait-descriptor.ts";
...
const { sessionService } = await import("./session.ts");
const { bus } = await import("../infra/events.ts");
const { config } = await import("../config/index.ts");
const dbOps = await import("../infra/db/index.ts");

// NOTE: we deliberately do NOT import from "./orchestrator.ts" in this
// test file. Other test files (`src/tools/delegate.test.ts`) install
// process-wide `mock.module("./orchestrator.ts", ...)` stubs, and Bun's
...
async function localExecuteRun(
  ...
  const { llm } = await import("../llm/llm.ts");
